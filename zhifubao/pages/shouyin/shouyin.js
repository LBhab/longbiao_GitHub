...
onLoad(query) {
  console.log(query);
     var _that = this;
     _that.setData({
       headerType: query.headertype,//头部是否显示
       payTpye: query.type,//按钮判断
     }); 
    console.log(query.orderNo);//订单号
         let pattern = /(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/;
        let timer = query.time.replace(pattern, '$1-$2-$3 $4:$5:$6');
        console.log(timer)
    setInterval(()=>{
      this.backTime(timer);  
    ...
    app.huoqu().then(res=>{
      ...
      this.getProducts(res.data, query.orderNo);//调起产品数据
    })
},
onReady() {
// 页面加载完成
